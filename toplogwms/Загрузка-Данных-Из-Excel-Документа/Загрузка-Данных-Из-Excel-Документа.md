...
Получение файлов происходит по участнику АОД **Получатель** по пути **Каталог входящий сообщений**. Собирает все существующие файлы из данного каталога (**так же вложенные файлы**). Если в настройке указана маска для файлов, то проверяется соответствие на маску по каждому файлу.

По окончанию обработки пользователю покажется сообщение о том, что загрузка окончена. Так же будет показано количество обработанных файлов и объектов.
